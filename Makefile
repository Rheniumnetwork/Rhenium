...
pkgdatadir = $(datadir)/Rhenium
pkgincludedir = $(includedir)/Rhenium
pkglibdir = $(libdir)/Rhenium
pkglibexecdir = $(libexecdir)/Rhenium
...
DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/Makefile.am \
	...
	$(top_srcdir)/src/config/Rhenium-config.h.in \
...
CONFIG_HEADER = $(top_builddir)/src/config/Rhenium-config.h
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_Rhenium_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_Rhenium_coverage.info test_Rhenium.info
...
src/config/Rhenium-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/Rhenium-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/Rhenium-config.h
$(top_srcdir)/src/config/Rhenium-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/Rhenium-config.h src/config/stamp-h1
...
#test_Rhenium.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_Rhenium -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_Rhenium_filtered.info: test_Rhenium.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_Rhenium_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_Rhenium_coverage.info: baseline_filtered_combined.info test_Rhenium_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_Rhenium_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_Rhenium_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_Rhenium_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_Rhenium.coverage/.dirstamp:  test_Rhenium_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
clean-local:
	rm -rf test_Rhenium.coverage/ total.coverage/ $(OSX_APP)
