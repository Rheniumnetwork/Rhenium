...
// Possible states for zXRH send
enum ZerocoinSpendStatus {
    ZXRH_SPEND_OKAY = 0,                            // No error
    ZXRH_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXRH_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXRH_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXRH_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXRH_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXRH_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXRH_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXRH_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXRH_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZXRH_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXRH_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
...
public:
    ...
    */
    ...
    void setZRheniumAutoBackups(bool fEnabled)
    {
