...
#include "zRheniumcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXRH ought to be enough for anybody." - Bill Gates, 2017
    ui->zXRHpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzXRHSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzXRH->setEnabled(false);
        ui->pushButtonMintzXRH->setToolTip(tr("zXRH is currently disabled due to maintenance."));

        ui->pushButtonSpendzXRH->setEnabled(false);
        ui->pushButtonSpendzXRH->setToolTip(tr("zXRH is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXRHpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXRH_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXRH is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXRH...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXRH in ") +
...
void PrivacyDialog::on_pushButtonSpendzXRH_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXRH is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zXRH
        sendzXRH();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXRH
    sendzXRH();
}

void PrivacyDialog::on_pushButtonZRheniumControl_clicked()
{
    ZRheniumControlDialog* zRheniumControl = new ZRheniumControlDialog(this);
    zRheniumControl->setModel(walletModel);
    zRheniumControl->exec();
}

void PrivacyDialog::setZRheniumControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzRheniumSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXRH()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Rhenium Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXRHpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zXRH is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XRH </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXRH</b>";
    ...
    // use mints from zRhenium selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZRheniumControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZRheniumControlDialog::GetSelectedMints();
    }
    ...
    // Clear zRhenium selector in case it was used
    ZRheniumControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zRhenium Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Rhenium, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zRhenium Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXRH </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXRH "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXRH "));
    ui->labelzXRHAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzXRHSyncStatus->setVisible(fShow);
}
