...
<<<<<<< HEAD
    widget->setPlaceholderText(QObject::tr("Enter a Rhenium address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
=======
    widget->setPlaceholderText(QObject::tr("Enter a Rhenium address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
>>>>>>> 3cb3aa92098e45afdbb5a3121b74b2ebf7e1705e
#endif
...
{
    // return if URI is not valid or is no Rhenium: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::XRH, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert Rhenium:// to Rhenium:
    //
    //    Cannot handle this later, because Rhenium:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::XRH, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open Rhenium.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Rhenium.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Rhenium.lnk
...
{
    return GetAutostartDir() / "Rhenium.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a Rhenium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Rhenium\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Rhenium app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add Rhenium app to startup item list
