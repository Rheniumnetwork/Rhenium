...
#include "ui_zRheniumcontroldialog.h"
...
std::list<std::string> ZRheniumControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZRheniumControlDialog::listMints;

ZRheniumControlDialog::ZRheniumControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZRheniumControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZRheniumControlDialog::~ZRheniumControlDialog()
{
    delete ui;
}

void ZRheniumControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRheniumControlDialog::updateList()
{
...
void ZRheniumControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRheniumControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZRhenium_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRheniumControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZRheniumControlDialog::GetSelectedMints()
{
...
void ZRheniumControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
