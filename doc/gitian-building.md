...
*Setup instructions for a gitian build of Rhenium using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Rhenium
...
- [Building Rhenium](#building-Rhenium)
...
Clone the git repositories for Rhenium and gitian and then checkout the Rhenium version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the Rhenium repository
...
To build Rhenium (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the Rhenium repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Rhenium/.git/
...
./bin/gbuild --commit Rhenium=${COMMIT} --url Rhenium=${URL} ../Rhenium/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Rhenium-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Rhenium-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/Rhenium-build.assert
```
